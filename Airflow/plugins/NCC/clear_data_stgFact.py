...
def clean_and_update_Stg_BaoCaoNCC_Fact():
    ...
    query = "SELECT * FROM Stg_BaoCaoNCC_Fact"
    ...
    for index, row in df.iterrows():
        ...
        try:
            ...
            print(f"Error processing fullName_NCC for Id_NCC and Id_Benef {row['Id_Benef'] and row['Id_NCC']}: {e}")
        
        try:
            update_columns.append('gender')
            update_values.append(row['gender'])
        except Exception as e:
            print(f"Error processing gender for Id_NCC and Id_Benef {row['Id_Benef'] and row['Id_NCC']}: {e}")
        
        try:
            update_columns.append('dateOfBirth')
            update_values.append(row['dateOfBirth'])
        except Exception as e:
            print(f"Error processing dateOfBirth for Id_NCC and Id_Benef {row['Id_Benef'] and row['Id_NCC']}: {e}")
        ...
        try:
            ...
            print(f"Error processing identityCard_NCC for Id_NCC and Id_Benef {row['Id_Benef'] and row['Id_NCC']}: {e}")
        ...
        try:
            ...
            print(f"Error processing nativePlace_NCC for Id_NCC and Id_Benef {row['Id_Benef'] and row['Id_NCC']}: {e}")
        
        try:
            update_columns.append('decided')
            update_values.append(row['decided'])
        except Exception as e:
            print(f"Error processing decided for Id_NCC and Id_Benef {row['Id_Benef'] and row['Id_NCC']}: {e}")
        
        try:
            update_columns.append('medicalCondition')
            update_values.append(row['medicalCondition'])
        except Exception as e:
            print(f"Error processing medicalCondition for Id_NCC and Id_Benef {row['Id_Benef'] and row['Id_NCC']}: {e}")
        
        # Cập nhật giá trị vào bảng nếu có cột nào hợp lệ
        if update_values:
            try:
                cursor.execute(f"""
                    UPDATE Stg_BaoCaoNCC_Fact
                    SET 
                        {', '.join([f"{col} = %s" for col in update_columns])}
                    WHERE Id_NCC = %s OR Id_Benef = %s
                """, tuple(update_values) + (row['Id_NCC'], row['Id_Benef']))
            except mysql.connector.Error as e:
                print(f"Error updating row with Id_Benef {row['Id_Benef'] and row['Id_NCC']}: {e}")
...
if __name__ == '__main__':
    clean_and_update_Stg_BaoCaoNCC_Fact()
